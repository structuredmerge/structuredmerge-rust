...
use ast_merge::{
    ...
    StructuredEditRequestEnvelope, StructuredEditResult, StructuredEditSelectionProfile,
    ...
    import_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_envelope,
    ...
    import_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope,
...
#[test]
fn conforms_to_slice_684_structured_edit_request_transport_envelope_fixture() {
    let fixture =
        read_fixture_from_path(diagnostics_fixture_path("structured_edit_request_envelope"));
    let request =
        serde_json::from_value::<StructuredEditRequest>(fixture["structured_edit_request"].clone())
            .expect("request should deserialize");
    let expected = serde_json::from_value::<StructuredEditRequestEnvelope>(
        fixture["expected_envelope"].clone(),
    )
    .expect("envelope should deserialize");

    assert_eq!(structured_edit_request_envelope(&request), expected);
    assert_eq!(import_structured_edit_request_envelope(&expected), Ok(request));
}

#[test]
fn conforms_to_slice_685_structured_edit_request_transport_rejection_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
        "structured_edit_request_envelope_rejection",
    ));
    let cases = fixture["cases"].as_array().expect("cases should be an array");

    for case in cases {
        let envelope =
            serde_json::from_value::<StructuredEditRequestEnvelope>(case["envelope"].clone())
                .expect("envelope should deserialize");
        let expected = serde_json::from_value::<StructuredEditTransportImportError>(
            case["expected_error"].clone(),
        )
        ...
        assert_eq!(import_structured_edit_request_envelope(&envelope), Err(expected));
    }
}

#[test]
fn conforms_to_slice_686_structured_edit_request_envelope_application_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
        "structured_edit_request_envelope_application",
    ));
    let envelope = serde_json::from_value::<StructuredEditRequestEnvelope>(
        fixture["structured_edit_request_envelope"].clone(),
    )
    .expect("envelope should deserialize");
    let expected =
        serde_json::from_value::<StructuredEditRequest>(fixture["expected_request"].clone())
            .expect("expected request should deserialize");

    assert_eq!(import_structured_edit_request_envelope(&envelope), Ok(expected));
    ...
    for case in cases {
        ...
        assert_eq!(
            import_structured_edit_request_envelope(&rejected_envelope),
            Err(expected_error)
        );
    }
}
...
#[test]
fn conforms_to_slice_661_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            for case in cases {
                let mut actual = serde_json::to_value(
                    serde_json::from_value::<
                        StructuredEditProviderExecutionReceiptReplayWorkflowApplyDecisionClosureReport,
                    >(
                        case["receipt_replay_workflow_apply_decision_closure_report"].clone()
                    )
                    .expect("apply decision closure report should deserialize"),
                )
                .expect("apply decision closure report should serialize");
                let mut expected =
                    case["receipt_replay_workflow_apply_decision_closure_report"].clone();
                prune_empty_metadata(&mut actual);
                prune_empty_metadata(&mut expected);
                assert!(
                    actual == expected,
                    "apply decision closure report payload should match fixture"
                );
            }
        })
...
#[test]
fn conforms_to_slice_662_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_transport_envelope_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            let report = serde_json::from_value::<
                StructuredEditProviderExecutionReceiptReplayWorkflowApplyDecisionClosureReport,
            >(
                fixture["structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report"].clone(),
            )
            .expect("apply decision closure report should deserialize");
            let expected = serde_json::from_value::<
                StructuredEditProviderExecutionReceiptReplayWorkflowApplyDecisionClosureReportEnvelope,
            ...
            assert_eq!(
                structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&report),
                expected
            );
            assert_eq!(
                import_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&expected)
                    .expect("apply decision closure report envelope should import"),
                report
            );
        })
        .expect("apply decision closure report transport envelope thread should spawn")
        .join()
        .expect("apply decision closure report transport envelope thread should complete");
}

#[test]
fn conforms_to_slice_663_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_transport_rejection_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            for case in cases {
                ...
                let actual = import_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&envelope)
                    .expect_err("closure report envelope import should fail");
                assert_eq!(actual, expected);
            }
        })
        .expect("apply decision closure report rejection thread should spawn")
        .join()
        .expect("apply decision closure report rejection thread should complete");
}

#[test]
fn conforms_to_slice_664_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope_application_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            let envelope = serde_json::from_value::<
                StructuredEditProviderExecutionReceiptReplayWorkflowApplyDecisionClosureReportEnvelope,
            >(
                fixture["structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope"].clone(),
            )
            .expect("envelope should deserialize");
            let expected = serde_json::from_value::<
                StructuredEditProviderExecutionReceiptReplayWorkflowApplyDecisionClosureReport,
            >(
                fixture["expected_receipt_replay_workflow_apply_decision_closure_report"].clone(),
            )
            .expect("expected closure report should deserialize");

            assert_eq!(
                import_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&envelope)
                    .expect("apply decision closure report envelope should import"),
                expected
            );
            ...
            for case in cases {
                ...
                let actual = import_structured_edit_provider_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&rejected)
                    .expect_err("closure report envelope application rejection should fail");
                assert_eq!(actual, expected);
            }
        })
        .expect("apply decision closure report application thread should spawn")
        .join()
        .expect("apply decision closure report application thread should complete");
}

#[test]
fn conforms_to_slice_665_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            for case in cases {
                let mut actual = serde_json::to_value(
                    serde_json::from_value::<
                        StructuredEditProviderBatchExecutionReceiptReplayWorkflowApplyDecisionClosureReport,
                    >(
                        case["batch_receipt_replay_workflow_apply_decision_closure_report"].clone()
                    )
                    .expect("batch apply decision closure report should deserialize"),
                )
                .expect("batch apply decision closure report should serialize");
                let mut expected =
                    case["batch_receipt_replay_workflow_apply_decision_closure_report"].clone();
                prune_empty_metadata(&mut actual);
                prune_empty_metadata(&mut expected);
                assert!(
                    actual == expected,
                    "batch apply decision closure report payload should match fixture"
                );
            }
        })
        .expect("batch apply decision closure report thread should spawn")
        .join()
        .expect("batch apply decision closure report thread should complete");
}

#[test]
fn conforms_to_slice_666_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_transport_envelope_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            let report = serde_json::from_value::<
                StructuredEditProviderBatchExecutionReceiptReplayWorkflowApplyDecisionClosureReport,
            >(
                fixture["batch_receipt_replay_workflow_apply_decision_closure_report"].clone(),
            )
            .expect("batch apply decision closure report should deserialize");
            let expected = serde_json::from_value::<
                StructuredEditProviderBatchExecutionReceiptReplayWorkflowApplyDecisionClosureReportEnvelope,
            ...
            assert_eq!(
                structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&report),
                expected
            );
            assert_eq!(
                import_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&expected)
                    .expect("batch apply decision closure report envelope should import"),
                report
            );
        })
        .expect("batch apply decision closure report transport envelope thread should spawn")
        .join()
        .expect("batch apply decision closure report transport envelope thread should complete");
}

#[test]
fn conforms_to_slice_667_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_transport_rejection_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            for case in cases {
                ...
                let actual = import_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&envelope)
                    .expect_err("batch closure report envelope import should fail");
                assert_eq!(actual, expected);
            }
        })
        .expect("batch apply decision closure report rejection thread should spawn")
        .join()
        .expect("batch apply decision closure report rejection thread should complete");
}

#[test]
fn conforms_to_slice_668_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_envelope_application_fixture()
 {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            ...
            let envelope = serde_json::from_value::<
                StructuredEditProviderBatchExecutionReceiptReplayWorkflowApplyDecisionClosureReportEnvelope,
            >(
                fixture["batch_receipt_replay_workflow_apply_decision_closure_report_envelope"].clone(),
            )
            .expect("envelope should deserialize");
            let expected = serde_json::from_value::<
                StructuredEditProviderBatchExecutionReceiptReplayWorkflowApplyDecisionClosureReport,
            >(
                fixture["expected_batch_receipt_replay_workflow_apply_decision_closure_report"].clone(),
            )
            .expect("expected batch closure report should deserialize");

            assert_eq!(
                import_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&envelope)
                    .expect("batch apply decision closure report envelope should import"),
                expected
            );
            ...
            for case in cases {
                ...
                let actual = import_structured_edit_provider_batch_execution_receipt_replay_workflow_apply_decision_closure_report_envelope(&rejected)
                    .expect_err("batch closure report envelope application rejection should fail");
                assert_eq!(actual, expected);
            }
        })
        .expect("batch apply decision closure report application thread should spawn")
        .join()
        .expect("batch apply decision closure report application thread should complete");
}
...
#[test]
fn conforms_to_slice_677_structured_edit_crispr_overmatch_fail_closed_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
        "structured_edit_crispr_overmatch_fail_closed",
    ));
    let cases = fixture["cases"].as_array().expect("cases should be an array");
    ...
    for case in cases {
        let mut actual = serde_json::to_value(
            serde_json::from_value::<StructuredEditExecutionReport>(case["report"].clone())
                .expect("crispr overmatch report should deserialize"),
        )
        .expect("crispr overmatch report should serialize");
        let mut expected = case["report"].clone();
        prune_empty_metadata(&mut actual);
        prune_empty_metadata(&mut expected);
        assert!(actual == expected, "crispr overmatch fail closed payload should match fixture");
    }
}
...
#[test]
fn conforms_to_slice_680_structured_edit_crispr_ruby_callable_destination_move_parity_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
        "structured_edit_crispr_ruby_callable_destination_move_parity",
    ));
    let cases = fixture["cases"].as_array().expect("cases should be an array");
    ...
    for case in cases {
        let mut actual = serde_json::to_value(
            serde_json::from_value::<StructuredEditExecutionReport>(case["report"].clone())
                .expect("crispr ruby callable destination move parity report should deserialize"),
        )
        .expect("crispr ruby callable destination move parity report should serialize");
        let mut expected = case["report"].clone();
        prune_empty_metadata(&mut actual);
        prune_empty_metadata(&mut expected);
        assert!(
            actual == expected,
            "crispr ruby callable destination move parity payload should match fixture"
        );
    }
}
...
#[test]
fn conforms_to_slice_703_ruby_gemspec_field_policy_acceptance_fixture() {
    let fixture =
        read_fixture_from_path(diagnostics_fixture_path("ruby_gemspec_field_policy_acceptance"));
    let cases = fixture["cases"].as_array().expect("cases should be an array");
    ...
    for case in cases {
        ...
        .expect("Ruby gemspec field policy report envelope should deserialize");

        assert!(report_envelope.report.final_content.contains("Real project summary"));
        assert!(!report_envelope.report.final_content.contains("spec.license ="));
    }
}
...
#[test]
fn conforms_to_slice_705_ruby_gemspec_files_policy_acceptance_fixture() {
    let fixture =
        read_fixture_from_path(diagnostics_fixture_path("ruby_gemspec_files_policy_acceptance"));
    let cases = fixture["cases"].as_array().expect("cases should be an array");
    ...
    for case in cases {
        ...
        .expect("Ruby gemspec files policy report envelope should deserialize");

        if case["label"] == "gemspec-files-literal-dir-union-and-duplicate-cleanup" {
            assert_eq!(report_envelope.report.final_content.matches("spec.files =").count(), 1);
            assert!(report_envelope.report.final_content.contains("sig/**/*.rbs"));
        }
    }
}

#[test]
fn conforms_to_slice_706_ruby_gemspec_version_loader_policy_acceptance_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
    ...
    for case in cases {
        ...
        .expect("Ruby gemspec version-loader policy report envelope should deserialize");

        if case["label"] == "modern-ruby-inline-version-loader" {
            assert!(!report_envelope.report.final_content.contains("gem_version ="));
        }
        if case["label"] == "legacy-ruby-gem-version-preamble" {
            assert!(report_envelope.report.final_content.contains("spec.version = gem_version"));
        }
    }
}
...
#[test]
fn conforms_to_slice_708_ruby_gemspec_self_dependency_policy_acceptance_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
        "ruby_gemspec_self_dependency_policy_acceptance",
    ));
    let cases = fixture["cases"].as_array().expect("cases should be an array");
    ...
    for case in cases {
        ...
        .expect("Ruby gemspec self-dependency policy report envelope should deserialize");

        if case["label"] == "delete-active-self-dependencies-preserve-comments" {
            assert!(
                !report_envelope
                    .report
                    .final_content
                    .contains("spec.add_dependency \"demo\", \"~> 1.0\"")
            );
            assert!(
                report_envelope
                    .report
                    .final_content
                    .contains("# spec.add_dependency \"demo\", \"~> 0\"")
            );
            assert_eq!(
...
#[test]
fn conforms_to_slice_709_ruby_gemfile_self_dependency_policy_acceptance_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
        "ruby_gemfile_self_dependency_policy_acceptance",
    ));
    let cases = fixture["cases"].as_array().expect("cases should be an array");
    ...
    for case in cases {
        ...
        .expect("Ruby Gemfile self-dependency policy report envelope should deserialize");

        if case["label"] == "delete-gemfile-self-dependencies-across-nesting" {
            assert!(!report_envelope.report.final_content.contains("gem \"demo\", \"~> 1.0\""));
            assert!(!report_envelope.report.final_content.contains("path: \"../dev/demo\""));
            assert!(report_envelope.report.final_content.contains("# gem \"demo\", \"~> 0\""));
            assert!(report_envelope.report.final_content.contains("gem \"fallback-gem\""));
            assert_eq!(
                report_envelope.report.step_reports[0].metadata["operation"].as_str(),
                Some("delete")
            );
...
#[test]
fn conforms_to_slice_711_ruby_appraisals_min_ruby_prune_policy_acceptance_fixture() {
    let fixture = read_fixture_from_path(diagnostics_fixture_path(
    ...
    for case in cases {
        ...
        .expect("Ruby Appraisals min-ruby prune policy report envelope should deserialize");

        if case["label"] == "delete-ruby-appraisals-below-min-ruby" {
            assert!(!report_envelope.report.final_content.contains("ruby-2-7"));
            assert!(!report_envelope.report.final_content.contains("ruby-3-0"));
            assert!(report_envelope.report.final_content.contains("ruby-3-2"));
            assert!(report_envelope.report.final_content.contains("appraise \"style\""));
            assert!(!report_envelope.report.final_content.contains("\n\n\n"));
        }
        if case["label"] == "missing-min-ruby-fails-closed" {
            assert_eq!(report_envelope.report.step_reports[0].status, "failed");
        }
    }
}
...
#[test]
fn conforms_to_slice_688_structured_edit_parity_match_semantics_fixture() {
    let fixture =
        read_fixture_from_path(diagnostics_fixture_path("structured_edit_parity_match_semantics"));
    let cases = fixture["cases"].as_array().expect("cases should be an array");

    for case in cases {
        let request = serde_json::from_value::<StructuredEditRequest>(case["request"].clone())
            .expect("parity match semantics request should deserialize");
        let round_tripped = serde_json::from_value::<StructuredEditRequest>(
            serde_json::to_value(&request)
                .expect("parity match semantics request should serialize"),
        )
        .expect("parity match semantics request should deserialize after roundtrip");
