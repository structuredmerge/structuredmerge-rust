...
use ast_template::{
    ...
    import_session_command_payload_envelope, import_session_diagnostics_envelope,
    import_session_entrypoint_envelope, import_session_inspection_envelope,
    ...
    import_session_runner_request_envelope, import_session_status_envelope,
    ...
    session_entrypoint_envelope, session_inspection_envelope, session_invocation_envelope,
    session_outcome_envelope, session_request_envelope, session_runner_payload_envelope,
    session_runner_request_envelope, session_status_envelope,
};
...
#[test]
fn conforms_to_template_directory_session_diagnostics_transport_envelope_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-416-template-directory-session-diagnostics-transport-envelope/template-directory-session-diagnostics-envelope.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let diagnostics: ast_template::SessionDiagnosticsReport =
            serde_json::from_value(test_case["input"].clone())
                .expect("diagnostics should deserialize");
        let expected: ast_template::SessionDiagnosticsEnvelope =
            serde_json::from_value(test_case["expected_envelope"].clone())
                .expect("expected envelope should deserialize");

        assert_eq!(session_diagnostics_envelope(&diagnostics), expected);
        assert_eq!(import_session_diagnostics_envelope(&expected), Ok(diagnostics));
    }
}
...
#[test]
fn conforms_to_template_directory_session_outcome_transport_rejection_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-408-template-directory-session-outcome-transport-rejection/template-directory-session-outcome-envelope-rejection.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionOutcomeEnvelope =
            serde_json::from_value(test_case["envelope"].clone())
                .expect("envelope should deserialize");
        let expected: ast_template::SessionOutcomeTransportImportError =
            serde_json::from_value(test_case["expected_error"].clone())
                .expect("expected error should deserialize");

        assert_eq!(import_session_outcome_envelope(&envelope), Err(expected));
    }
}

#[test]
fn conforms_to_template_directory_session_outcome_envelope_application_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-409-template-directory-session-outcome-envelope-application/template-directory-session-outcome-envelope-application.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionOutcomeEnvelope =
            serde_json::from_value(test_case["envelope"].clone())
                .expect("envelope should deserialize");
        let outcome =
            import_session_outcome_envelope(&envelope).expect("envelope import should succeed");
        let expected = decode_session_outcome_report(&test_case["expected"]);
        assert_eq!(
            serde_json::to_value(outcome).expect("outcome should serialize"),
            serde_json::to_value(expected).expect("expected outcome should serialize")
        );
    }
...
#[test]
fn conforms_to_template_directory_session_request_transport_envelope_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-404-template-directory-session-request-transport-envelope/template-directory-session-request-envelope.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let request = decode_session_request_report_from_fixture(&test_case["input"], fixture_root);
        let expected: ast_template::SessionRequestEnvelope =
            serde_json::from_value(resolve_session_request_envelope_fixture_paths(
                &test_case["expected_envelope"],
                fixture_root,
            ))
            .expect("expected envelope should deserialize");

        assert_eq!(session_request_envelope(&request), expected);
        assert_eq!(import_session_request_envelope(&expected), Ok(request));
    }
}

#[test]
fn conforms_to_template_directory_session_request_transport_rejection_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-405-template-directory-session-request-transport-rejection/template-directory-session-request-envelope-rejection.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionRequestEnvelope = serde_json::from_value(
            resolve_session_request_envelope_fixture_paths(&test_case["envelope"], fixture_root),
        )
        .expect("envelope should deserialize");
        let expected: ast_template::SessionRequestTransportImportError =
            serde_json::from_value(test_case["expected_error"].clone())
                .expect("expected error should deserialize");

        assert_eq!(import_session_request_envelope(&envelope), Err(expected));
    }
}

#[test]
fn conforms_to_template_directory_session_request_envelope_application_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-406-template-directory-session-request-envelope-application/template-directory-session-request-envelope-application.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        ...
        .expect("envelope should deserialize");
        let request =
            import_session_request_envelope(&envelope).expect("envelope import should succeed");
        let actual = run_template_directory_session_request(&request)
            .expect("request envelope application should succeed");
        assert_eq!(
            serde_json::to_value(actual).expect("outcome should serialize"),
            resolve_session_outcome_expected_paths(&test_case["expected"], fixture_root)
        );
    }
...
#[test]
fn conforms_to_template_directory_session_command_payload_transport_rejection_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-393-template-directory-session-command-payload-transport-rejection/template-directory-session-command-payload-envelope-rejection.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionCommandPayloadEnvelope =
            serde_json::from_value(resolve_session_command_payload_envelope_fixture_paths(
                &test_case["envelope"],
                fixture_root,
            ))
            .expect("envelope should deserialize");
        let expected: ast_template::SessionCommandPayloadTransportImportError =
            serde_json::from_value(test_case["expected_error"].clone())
                .expect("expected error should deserialize");

        assert_eq!(import_session_command_payload_envelope(&envelope), Err(expected));
    }
}
...
#[test]
fn conforms_to_template_directory_session_runner_request_transport_envelope_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-398-template-directory-session-runner-request-transport-envelope/template-directory-session-runner-request-envelope.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let request = decode_session_runner_request_from_fixture(&test_case["input"], fixture_root);
        let expected: ast_template::SessionRunnerRequestEnvelope =
            serde_json::from_value(resolve_session_runner_request_envelope_fixture_paths(
                &test_case["expected_envelope"],
                fixture_root,
            ))
            .expect("expected envelope should deserialize");

        assert_eq!(session_runner_request_envelope(&request), expected);
        assert_eq!(import_session_runner_request_envelope(&expected), Ok(request));
    }
}
...
#[test]
fn conforms_to_template_directory_session_runner_payload_transport_rejection_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-402-template-directory-session-runner-payload-transport-rejection/template-directory-session-runner-payload-envelope-rejection.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionRunnerPayloadEnvelope =
            serde_json::from_value(resolve_session_runner_payload_envelope_fixture_paths(
                &test_case["envelope"],
                fixture_root,
            ))
            .expect("envelope should deserialize");
        let expected: ast_template::SessionRunnerPayloadTransportImportError =
            serde_json::from_value(test_case["expected_error"].clone())
                .expect("expected error should deserialize");

        assert_eq!(import_session_runner_payload_envelope(&envelope), Err(expected));
    }
}
...
fn conforms_to_template_directory_session_runner_request_transport_rejection_fixture() {
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionRunnerRequestEnvelope =
            serde_json::from_value(resolve_session_runner_request_envelope_fixture_paths(
                &test_case["envelope"],
                fixture_root,
            ))
            .expect("envelope should deserialize");
        let expected: ast_template::SessionRunnerRequestTransportImportError =
            serde_json::from_value(test_case["expected_error"].clone())
                .expect("expected error should deserialize");

        assert_eq!(import_session_runner_request_envelope(&envelope), Err(expected));
    }
}

#[test]
fn conforms_to_template_directory_session_entrypoint_transport_rejection_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-396-template-directory-session-entrypoint-transport-rejection/template-directory-session-entrypoint-envelope-rejection.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionEntrypointEnvelope = serde_json::from_value(
            resolve_session_entrypoint_envelope_fixture_paths(&test_case["envelope"], fixture_root),
        )
        .expect("envelope should deserialize");
        let expected: ast_template::SessionEntrypointTransportImportError =
            serde_json::from_value(test_case["expected_error"].clone())
                .expect("expected error should deserialize");

        assert_eq!(import_session_entrypoint_envelope(&envelope), Err(expected));
    }
}
...
fn conforms_to_template_directory_session_command_envelope_application_fixture() {
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        ...
        .expect("envelope should deserialize");
        let command =
            import_session_command_envelope(&envelope).expect("envelope import should succeed");
        let actual = run_template_directory_session_command(&command, &profiles)
            .expect("command envelope application should succeed");
        assert_eq!(
...
#[test]
fn conforms_to_template_directory_session_command_payload_envelope_application_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-394-template-directory-session-command-payload-envelope-application/template-directory-session-command-payload-envelope-application.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionCommandPayloadEnvelope =
            ...
            .expect("envelope should deserialize");
        let payload = import_session_command_payload_envelope(&envelope)
            .expect("envelope import should succeed");
        let actual = run_template_directory_session_command_payload(&payload, &profiles)
            .expect("command payload envelope application should succeed");
        assert_eq!(
            serde_json::to_value(actual).expect("dispatch report should serialize"),
            resolve_session_dispatch_expected_paths(&test_case["expected"], fixture_root)
        );
...
#[test]
fn conforms_to_template_directory_session_entrypoint_envelope_application_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-397-template-directory-session-entrypoint-envelope-application/template-directory-session-entrypoint-envelope-application.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        ...
        .expect("envelope should deserialize");
        let entrypoint =
            import_session_entrypoint_envelope(&envelope).expect("envelope import should succeed");
        let actual = run_template_directory_session_entrypoint(&entrypoint, &profiles)
            .expect("entrypoint envelope application should succeed");
        assert_eq!(
            serde_json::to_value(actual).expect("outcome should serialize"),
            resolve_session_outcome_expected_paths(&test_case["expected"], fixture_root)
        );
...
#[test]
fn conforms_to_template_directory_session_runner_request_envelope_application_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-400-template-directory-session-runner-request-envelope-application/template-directory-session-runner-request-envelope-application.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionRunnerRequestEnvelope =
            ...
            .expect("envelope should deserialize");
        let request = import_session_runner_request_envelope(&envelope)
            .expect("envelope import should succeed");
        let actual = run_template_directory_session_runner_request(&request, &profiles)
            .expect("runner request envelope application should succeed");
        assert_eq!(
            serde_json::to_value(actual).expect("outcome should serialize"),
            resolve_session_outcome_expected_paths(&test_case["expected"], fixture_root)
        );
...
#[test]
fn conforms_to_template_directory_session_runner_payload_envelope_application_fixture() {
    let fixture_path = repo_root().join(
        "fixtures/diagnostics/slice-403-template-directory-session-runner-payload-envelope-application/template-directory-session-runner-payload-envelope-application.json",
    );
    let fixture: Value =
        serde_json::from_slice(&fs::read(&fixture_path).expect("fixture should be readable"))
            .expect("fixture should deserialize");
    ...
    for test_case in fixture["cases"].as_array().expect("cases should be array") {
        let envelope: ast_template::SessionRunnerPayloadEnvelope =
            ...
            .expect("envelope should deserialize");
        let payload = import_session_runner_payload_envelope(&envelope)
            .expect("envelope import should succeed");
        let actual = run_template_directory_session_runner_payload(&payload, &profiles)
            .expect("runner payload envelope application should succeed");
        assert_eq!(
            serde_json::to_value(actual).expect("outcome should serialize"),
            resolve_session_outcome_expected_paths(&test_case["expected"], fixture_root)
        );
...
fn resolve_session_request_envelope_fixture_paths(
    fixture: &Value,
    fixture_root: &std::path::Path,
) -> Value {
    let mut resolved = fixture.clone();
    if let Some(section) = resolved.as_object_mut() {
        if let Some(request) = section.get_mut("request") {
            *request = serde_json::to_value(decode_session_request_report_from_fixture(
                request,
                fixture_root,
            ))
...
fn resolve_session_runner_request_envelope_fixture_paths(
    fixture: &Value,
    fixture_root: &std::path::Path,
) -> Value {
    let mut resolved = fixture.clone();
    if let Some(section) = resolved.as_object_mut() {
        if let Some(request) = section.get_mut("request") {
            *request = serde_json::to_value(decode_session_runner_request_from_fixture(
                request,
                fixture_root,
            ))
            .expect("request should serialize");
...
fn resolve_session_outcome_expected_paths(value: &Value, fixture_root: &std::path::Path) -> Value {
    let mut resolved = value.clone();
    if let Some(section) = resolved.as_object_mut() {
        if let Some(session_report) =
            section.get_mut("session_report").and_then(Value::as_object_mut)
        {
            if let Some(mode) = session_report.get("mode").cloned() {
                session_report.insert("mode".to_string(), mode);
            }
        }
    }
    let _ = fixture_root;
